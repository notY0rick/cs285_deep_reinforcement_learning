...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # return normalized delta_pred_normalized and unnormalized next_obs_pred
        return next_obs_pred, delta_pred_normalized  # returns a torch.Tensor tuple
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = self.forward(obs, acs,\
                                    ...
                                    ptu.from_numpy(data_statistics['delta_std']))[0]  # TODO(Q1) get the predicted next-states (s_t+1) as a numpy array
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        return ptu.to_numpy(prediction) # returns numpy array (default)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        preds = self.forward(observations, actions, ptu.from_numpy(data_statistics['obs_mean']), ptu.from_numpy(data_statistics['obs_std']),\
                                                    ptu.from_numpy(data_statistics['acs_mean']), ptu.from_numpy(data_statistics['acs_std']),\
                                                    ptu.from_numpy(data_statistics['delta_mean']), ptu.from_numpy(data_statistics['delta_std']))[1]
        assert target.shape == preds.shape, f"Target {target.shape} and prediction {preds.shape} arrays have different shapes!"
        ...
        # self.update_statistics(data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'],\
