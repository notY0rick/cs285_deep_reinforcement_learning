...
class IQLAgent(DQNAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            self.running_rnd_rew_std = (1 - self.rnd_gamma) * np.std(expl_bonus, axis=0) + self.rnd_gamma * self.running_rnd_rew_std
            # expl_bonus = normalize(expl_bonus, np.mean(expl_bonus, axis=0), np.std(expl_bonus, axis=0))
