...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        """
        return torch.abs(self.iql_expectile - (diff < 0).float()) * (diff * diff)
    ...
    def update_v(self, ob_no, ac_na):
        ...
        q_values = torch.gather(qa_values, 1, ac_na.unsqueeze(1).type(torch.int64)).squeeze(1)
        v_values = self.v_net(ob_no).squeeze()
        assert q_values.shape == v_values.shape, "q_values and v_values not equal shapes!"
        exp_loss = self.expectile_loss(q_values - v_values)

        value_loss = torch.mean(exp_loss, dim=0).squeeze()
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        q_values = torch.gather(qa_values, 1, ac_na.unsqueeze(1).type(torch.int64)).squeeze(1)
        v_values = self.v_net(next_ob_no).squeeze()
        target = reward_n + self.gamma * v_values * (1 - terminal_n)
