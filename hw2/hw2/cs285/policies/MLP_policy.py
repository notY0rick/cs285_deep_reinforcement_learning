...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions, **kwargs):
        ...
        loss = None
        if self.discrete:
            loss = nn.CrossEntropyLoss(pred_actions.rsample(), actions)
        else:
            loss = nn.MSELoss(pred_actions.rsample(), actions)
        loss.backward()
        self.optimizer.step()
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        # pred_actions_log_prob = dist.log_prob(pred_actions)
        actions_log_prob = dist.log_prob(actions)
        # loss = torch.sum(advantages * pred_actions_log_prob, dim=1)
        # print(advantages.shape)
        # print(actions_log_prob.shape)
        loss = -1 * torch.sum(advantages * actions_log_prob)
